...
public class BasicBinPacker {
	...
	protected void setBound(int bt){
		boundType_ = bt;
	}
	...
	public void ext(  int numLarge, int numSmall, double minLargeUtil,  double maxLargeUtil){
		double maxSmallUtil = .6;
		double r;
		//itemsToPack_.clear();
		itemsToPack_ = new ArrayList();
		for(int i = 0; i < numLarge; i++){
			String itemName = "Big Item " + i;
			r = Math.random();
			double util = r*(maxLargeUtil - minLargeUtil) + minLargeUtil;
			ClassicItem ci = new ClassicItem(i,itemName, new double[] {util}, 5);
			itemsToPack_.add(ci);
		}
		for(int i = 0; i < numSmall; i++){
			String itemName = "Little Item " + i;
			r = Math.random();
			double util = r* maxSmallUtil;
			ClassicItem ci = new ClassicItem(i,itemName, new double[] {util}, 5);
			itemsToPack_.add(ci);
		}
		bins_ = new ArrayList();
		//bins_.clear();
		setBins(numLarge+numSmall);
	...
	public void setBins(int numBins){
		...
		for(int i = 0; i < (numBins); i++){
			...
			ClassicBin b = new ClassicBin(1, "Bin "+i, sl);
			//System.out.println(" adding bin " + b.getBinName_());
			bins_.add(b);
	...
	public void writeResults(String fileName, List<List<ClassicBin>> listsOfBins ){
		...
		try {
			...
			for(List<ClassicBin> binList : listsOfBins){
				for(ClassicBin b : binList){
					int sumTasks = 0;
					...
					out.write(sumTasks+"," );
					System.out.print(b.getItems().size()+",");
				}
				out.write("\n");
			}
				out.write("\n");
	...
	public static void main(String args[]){
		...
		bins.add(bin4);
		bins.add(bin5);*/
		BasicBinPacker bfp = new BasicBinPacker(items,bins,0); 
		/*
		 * The third option is which type of bound we are setting
		 * 0 = .694, 1 = LLBound, 2= 1 (Harmonic 100 %)
		 */
		List<List<ClassicBin>> masterList = new ArrayList();
		...
		double maxUtil = .4;
		double minUtil = .1;
		for(double i = minUtil; i < .5; i = i+.1){
			bfp.ext(numSmall, numLarge, minUtil, maxUtil);
			List<ClassicItem> bfpItems = bfp.getItems();
			for(int bt = 0; bt <3; bt++){
				bfp.setBound(bt);
				bfp.setBins(numLarge + numSmall);
				bfp.setItems(bfpItems);
				System.out.println(" bound is " + bfp.getBound(new ClassicBin(), new ClassicItem(-1,"ffake")));
				bfp.packItems();
				doneBins = null;
				doneBins = new ArrayList(bfp.getBins());
				//for(ClassicBin b : doneBins){
					//System.out.print((1-b.getSpaceLeft_()[0])+"," );
				//}
				masterList.add(doneBins);
				System.out.println();
		...
		for(List<ClassicBin> db : masterList){
			for(ClassicBin b : db){
		...
		bfp.writeResults("IcseExt.txt",masterList);
	/*	for(ClassicBin b : doneBins){
			int sumTasks = 0;
			for(ClassicItem i : b.getItems()){
				//System.out.println( b.getBinName_() + " has item " + i.getName() + " with " + i.getNumTasks_() + " tasks");
