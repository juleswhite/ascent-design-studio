 ...
 ******************************************************************************/
public class SubFeaturesHandler implements ConfigDirectiveHandler {
	...
	public void handle(RefreshProblem problem, String context,
		...
		if (rfeatures != null && rfeatures.length > 0) {
			...
			for (String rfeat : rfeatures) {
				req.add(rfeat.trim());
				problem.getRequires(rfeat).add(context.trim());
			}
			problem.getRequires(context.trim()).addAll(req);
		}
	}
