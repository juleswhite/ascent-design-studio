...
public class TemplateParser {
	...
	public static Template parse(ModelHelper mh, String tstr) {
		...
		if (eptype == null)
			throw new TemplateParseException(
					"The root element of each template must specify a value for the attribute \""
							+ Template.TYPE_ATTR + "\"");
		...
		template.setStylePrimaryName(e.attributes
				.get(Template.STYLE_PRIMARY_NAME));
		template.setResizable("true".equalsIgnoreCase(e.attributes
				.get(Template.RESIZABLE)));
		template.setMoveable("true".equalsIgnoreCase(e.attributes
				.get(Template.MOVEABLE)));
		template.setRootAttributes(e.attributes);
		return template;
	}

	public static Token parseDirective(String t, String directivename) {
		String key = "<!--" + directivename;
		Token token = null;
		int start = t.indexOf(key);
		if (start > -1) {
			int end = t.indexOf("-->", start + key.length());
			if (end > -1) {
				token = new Token();
				token.remainder = t.substring(end + 3);
				token.data = t.substring(start + key.length(), end);
			}
		}
		return token;
	}

	public static Token parseAnyElement(String t) {
		int start = t.indexOf("<");
		if (start < 0)
	...
	public static Map<String, String> parseAttributes(String e) {
	...
	public static Map<String, String> parseMap(String e) {
		if (e == null || e.length() < 1)
			return new HashMap<String, String>();
