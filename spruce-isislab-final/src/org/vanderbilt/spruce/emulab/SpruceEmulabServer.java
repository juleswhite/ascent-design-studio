...
public class SpruceEmulabServer {
	...
	public void start(int port, String url, String user, String pass) {
		...
		context.addServlet(new ServletHolder(new SpruceParamsServlet(url)), "/validate"); // Validates login and serves up list of parameterizable experiments
		context.addServlet(new ServletHolder(new SpruceConfigServlet(url)), "/config");//If an experiment is configurable, loads experiment configuration form
		...
		handlers.setHandlers(new Handler[] { resource_handler, context });//loads all servlet handlers
